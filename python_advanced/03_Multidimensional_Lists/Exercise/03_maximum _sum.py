...
maximum_sum, greatest_3x3_matrix = -50, list()  # the negative matrix dot is max -50
...
for row in range(rows-2):
    for col in range(cols-2):
        ...
        if sum(template.values()) > maximum_sum:
            maximum_sum = sum(template.values())
            greatest_3x3_matrix = template

print(expected_output(max_sum=maximum_sum, greatest_3x3=greatest_3x3_matrix))
