...
def get_toy_by(magic_value):
    for toy, magic_price in toy_pricing.items():
...
# print(f"Materials left: {', '.join(list(str(i) for i in reversed(materials)))}" if materials
#       else f"Magic left: {', '.join(list(str(i) for i in reversed(magic_values)))}")
