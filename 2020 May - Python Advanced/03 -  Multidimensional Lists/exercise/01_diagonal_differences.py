...
diagonal_started_top_left = [(matrix[i][i]) for i in range(len(matrix))]
horizontal = [h for h in range(len(matrix))]
vertical = [v for v in range(len(matrix) - 1, -1, -1)]
diagonal_started_top_right = [matrix[i[0]][i[1]] for i in tuple(zip(horizontal, vertical))]
print(abs(sum(diagonal_started_top_left) - sum(diagonal_started_top_right)))
